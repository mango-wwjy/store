package com.shxt.modual.index.model;

/**
 * 主页
 * @author
 * @Version 1.0
 * @Copyright
 * @date  下午6:47:34
 * @description
 */
public class Index {
    private String top1;
    private String top2;
    private String top3;
    private String mid1;
    private String mid2;
    private String mid3;
    private String mid4;
    private String woman1;
    private String woman2;
    private String woman3;
    private String man1;
    private String man2;
    private String man3;
    public String getTop1() {
        return top1;
    }
    public void setTop1(String top1) {
        this.top1 = top1;
    }
    public String getTop2() {
        return top2;
    }
    public void setTop2(String top2) {
        this.top2 = top2;
    }
    public String getTop3() {
        return top3;
    }
    public void setTop3(String top3) {
        this.top3 = top3;
    }
    public String getMid1() {
        return mid1;
    }
    public void setMid1(String mid1) {
        this.mid1 = mid1;
    }
    public String getMid2() {
        return mid2;
    }
    public void setMid2(String mid2) {
        this.mid2 = mid2;
    }
    public String getMid3() {
        return mid3;
    }
    public void setMid3(String mid3) {
        this.mid3 = mid3;
    }
    public String getMid4() {
        return mid4;
    }
    public void setMid4(String mid4) {
        this.mid4 = mid4;
    }
    public String getWoman1() {
        return woman1;
    }
    public void setWoman1(String woman1) {
        this.woman1 = woman1;
    }
    public String getWoman2() {
        return woman2;
    }
    public void setWoman2(String woman2) {
        this.woman2 = woman2;
    }
    public String getWoman3() {
        return woman3;
    }
    public void setWoman3(String woman3) {
        this.woman3 = woman3;
    }
    public String getMan1() {
        return man1;
    }
    public void setMan1(String man1) {
        this.man1 = man1;
    }
    public String getMan2() {
        return man2;
    }
    public void setMan2(String man2) {
        this.man2 = man2;
    }
    public String getMan3() {
        return man3;
    }
    public void setMan3(String man3) {
        this.man3 = man3;
    }
    @Override
    public String toString() {
        return "Index [top1=" + top1 + ", top2=" + top2 + ", top3=" + top3 + ", mid1=" + mid1 + ", mid2=" + mid2
                + ", mid3=" + mid3 + ", mid4=" + mid4 + ", woman1=" + woman1 + ", woman2=" + woman2 + ", woman3="
                + woman3 + ", man1=" + man1 + ", man2=" + man2 + ", man3=" + man3 + "]";
    }

}
